...
function Navbar() {
    ...
    const handleRegister = async () => {
        ...
        try {
            // Envía una solicitud POST a la API de registro con los datos de usuario
            ...
            if (response.ok) {
                setSuccess("Usuario registrado exitosamente"); // Muestra el mensaje de éxito
                
                setTimeout(() => {
                    setShowSecondaryRegisterModal(false);  // Cierra el modal después de un pequeño delay
                }, 2000);  // Cerrar el modal 2 segundos después de mostrar el mensaje
            } else {
                // Si hay un error del servidor, mostramos el mensaje de error específico
                setError(data.error || "Error al registrar usuario");
            }
        } catch (error) {
            console.error("Error de red:", error);
            setError("Error en la conexión con el servidor"); // Error de red o conexión
        } finally {
    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            const data = await response.json(); // Consumir el cuerpo de la respuesta una vez
        
            if (response.ok) {
                const { token, tipo_usuario } = data;
        
                // Almacena el token y tipo de usuario en el localStorage
                localStorage.setItem('token', token);
                localStorage.setItem('tipo_usuario', tipo_usuario);
                localStorage.setItem('correo', correo); // Almacena el correo en localStorage
                ...
                setError(data.error || "Error al iniciar sesión"); // Maneja el error de respuesta
            }
        } catch (error) {
            console.error("Error de red: ", error);
    ...
    const handleCloseSecondaryModal = () => setShowSecondaryModal(false);  // Cierra el modal secundario
    ...
    const handleCloseSecondaryRegisterModal = () => setShowSecondaryRegisterModal(false);  // Cierra el modal de registro secundario
